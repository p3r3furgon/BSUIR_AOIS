...
namespace AOIS_1
{
    internal class BinaryDecimalConverter
    {
        ...
        //    int integerValuesLength = 16, numberOfSums = 4;
        ...
        //        {    
        //            sum = Sum(PresentInDirectCode(intValue1, integerValuesLength), PresentInDirectCode(intValue2, integerValuesLength));
        //            Console.WriteLine("value1 = " + intValue1 + " = " + PresentInDirectCode(intValue1, integerValuesLength) + ", value2 = " + intValue2 + " = " + PresentInDirectCode(intValue2, integerValuesLength) + ", sum = " + sum + " = " + ConvertFromBinaryToInt(sum));
        //        }
        //        else
        //        {
        //            sum = DifferenceWithVariousSigns(PresentInDirectCode(intValue1, integerValuesLength), PresentInDirectCode(intValue2, integerValuesLength));
        //            Console.WriteLine("value1 = " + intValue1 + " = " + PresentInDirectCode(intValue1, integerValuesLength) + ", value2 = " + intValue2 + " = " + PresentInDirectCode(intValue2, integerValuesLength) + ", sum = " + sum + " = " + ConvertFromBinaryToInt(sum));
        //        }
        //    }
        //    multiplication = Multiplication(PresentInDirectCode(value1, integerValuesLength), PresentInDirectCode(value2, integerValuesLength));
        //    division = Division(PresentInDirectCode(value1, integerValuesLength), PresentInDirectCode(value2, integerValuesLength));
        ...
        public static string ConvertToBinary(int value, int numberOfBits)
        {
            var zerosPart = (numberOfBits - Convert.ToString(Math.Abs(value), 2).Length);
            var binaryValue = "";
            for (int i = 0; i < zerosPart; i++)
                binaryValue += "0";
            return binaryValue + Convert.ToString(value, 2);
        }
        ...
        public static int ConvertPositiveBinaryValue(string binaryValue)
        {
            int result = 0;
        ...
        static string ConvertFromDecimalToBinary(decimal value)
        {
            ...
            int order = 0, mantissasLength = 23;
            if(value >= 0) 
                sign = "0";
            else
                sign = "1";
            ...
            for(int i = 0; i < mantissasLength - fractalBinaryForm.Length; i++) 
                fractalPartZeros += '0';
            fractalBinaryForm += fractalPartZeros;
            fractalBinaryForm = fractalBinaryForm.Remove(mantissasLength, fractalBinaryForm.Length - mantissasLength);
        ...
        {
            int centralBit = 127, exponentsLength = 8;
            return PresentInDirectCode(order + centralBit, exponentsLength);
        }
        ...
        {
            int signsLength = 1, exponentsLength = 8, mantissasLength = 23;
            string[] signs = { binaryValue1[0].ToString(), binaryValue2[0].ToString() };
            string[] exponents = { binaryValue1.Substring(signsLength, exponentsLength), binaryValue2.Substring(signsLength, exponentsLength) };
            string[] mantissas = { binaryValue1.Substring(signsLength+exponentsLength,mantissasLength), binaryValue2.Substring(signsLength+exponentsLength,mantissasLength)};
            string[] unseenBits = { "1", "1" };
            int shiftValue = Math.Abs(ConvertFromBinaryToInt(DifferenceWithSameSigns(signs[0] + exponents[0], signs[1] + exponents[1],signsLength+exponentsLength)));
        ...
        static string SumOfMantissas(string[] mantissas, string[] unseenBits, ref string exponent, string sign)
        {
            ...
            {
                exponent = Sum(sign + exponent, PresentInDirectCode(1, (sign+exponent).Length)).Remove(0,1);
        ...
        {
            int exponentsLength = 8, mantissasLength = 23, centralBit = 127;
            string sign = binaryValue[0].ToString();
            string exponent = binaryValue.Substring(1, exponentsLength);
            string mantissa = "1" + binaryValue.Substring(9, mantissasLength);
            int decimalDegree = ConvertFromBinaryToInt(sign + exponent) - centralBit;
